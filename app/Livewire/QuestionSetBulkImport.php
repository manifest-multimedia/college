...
class QuestionSetBulkImport extends Component
{
    ...
    private function parseExcelRow($row, $lineNumber)
    {
        ...
        // Validate options
        $options = [];
        if (!empty($optionOne)) $options[] = ['text' => $optionOne, 'label' => 'A'];
        if (!empty($optionTwo)) $options[] = ['text' => $optionTwo, 'label' => 'B'];
        if (!empty($optionThree)) $options[] = ['text' => $optionThree, 'label' => 'C'];
        if (!empty($optionFour)) $options[] = ['text' => $optionFour, 'label' => 'D'];
