...
class StudentImporter implements ToCollection, WithBatchInserts, WithChunkReading, WithHeadingRow, WithValidation
{
    protected $programId;
    ...
    protected function mapRowToStudentData($row)
    {
        ...
        foreach ($this->columnMapping as $excelColumn => $dbField) {
            ...
            if (isset($row[$normalizedExcelColumn])) {
                ...
                // Sanitize email by removing all spaces
                if ($dbField === 'email' && ! is_null($value)) {
                    $value = str_replace(' ', '', $value);
                }

                // Convert Excel date serial number to proper date format
                if ($dbField === 'date_of_birth' && ! is_null($value)) {
                    ...
                    if (is_numeric($value)) {
                        try {
                            ...
                            Log::warning("Failed to convert Excel date: {$value}");
                        }
                    }
                }

                // Normalize gender to lowercase for consistency
                if ($dbField === 'gender' && ! is_null($value)) {
    ...
    protected function validateStudentData($studentData, $rowNumber)
    {
        ...
        if (! empty($studentData['gender'])) {
            ...
            if (! in_array(strtolower($studentData['gender']), $validGenders)) {
