...
{
    /**
     * Detect the column type of cohorts.id to ensure FK compatibility
     */
    private function getCohortsIdColumnType(): string
    {
        ...
        if (empty($column)) {
            // Default to unsignedInteger if cohorts table doesn't exist yet
            return 'unsignedInteger';
        }

        $columnType = strtolower($column[0]->COLUMN_TYPE);
    ...
    {
        // Detect the correct column type for cohort_id
        $columnType = $this->getCohortsIdColumnType();

        Schema::table('assessment_scores', function (Blueprint $table) use ($columnType) {

            // -----------------------------
            // ADD NEW COLUMN (if not exists)
            // -----------------------------
            if (! Schema::hasColumn('assessment_scores', 'cohort_id')) {
                // Use dynamic column type to match cohorts.id
        ...
        // -----------------------------
        // FIX COLUMN TYPE IF IT EXISTS BUT IS WRONG TYPE
        // -----------------------------
        // Check if cohort_id exists with wrong type and fix it
        $connection = DB::connection();
        $databaseName = $connection->getDatabaseName();

        $existingColumn = DB::select("
            SELECT DATA_TYPE, COLUMN_TYPE 
            FROM INFORMATION_SCHEMA.COLUMNS 
            WHERE TABLE_SCHEMA = ? 
        ...
        if (! empty($existingColumn)) {
            $existingType = strtolower($existingColumn[0]->COLUMN_TYPE);
            $expectedType = $columnType === 'unsignedBigInteger' ? 'bigint' : 'int';

            // If types don't match, we need to modify the column
            if (! str_contains($existingType, $expectedType)) {
                Schema::table('assessment_scores', function (Blueprint $table) use ($columnType) {
                    if ($columnType === 'unsignedBigInteger') {
                        $table->unsignedBigInteger('cohort_id')->nullable()->change();
                    } else {
                        $table->unsignedInteger('cohort_id')->nullable()->change();
                    }
                });
            }
        }
        ...
        $defaultCohortId = DB::table('cohorts')->value('id'); // Fetch the first cohort ID
        if ($defaultCohortId) {
            DB::table('assessment_scores')->whereNull('cohort_id')->update(['cohort_id' => $defaultCohortId]);
        } else {
            // If no cohorts exist, create a placeholder cohort and use its ID
            $defaultCohortId = DB::table('cohorts')->insertGetId([
                'name' => 'Default Cohort',
